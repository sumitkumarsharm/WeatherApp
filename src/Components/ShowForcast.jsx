...
const ShowForecast = ({ city }) => {
  ...
  if (foreCastLoading)
    return <p className="text-white text-center">Loading forecast...</p>;
  ...
  return (
    <div className="w-full px-4 py-6">
      <div className="max-w-3xl mx-auto flex flex-col gap-4">
        ...
        {/* Forecast Header */}
        <div className="hidden sm:flex justify-between text-white font-semibold px-2">
          <p>Date</p>
          <p>Temperature</p>
        </div>
        ...
        <div className="flex flex-col gap-3">
          {visibleForecast.map((day, index) => (
            <ForecastCard key={index} data={day} formatDate={formatDate} />
          ))}
        </div>
